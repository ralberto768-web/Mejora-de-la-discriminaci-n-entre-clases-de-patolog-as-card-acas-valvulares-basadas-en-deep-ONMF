...
1. Lee las caracteristicas log-mel guardadas en NPZ.
2. Interpreta cada columna temporal del espectrograma como paso de secuencia.
3. Entrena una LSTM de dos capas.
4. Guarda el modelo y las metricas en texto.

Ejecucion manual:
    python 4-entrenar_lstm.py --duracion 2.0 --epocas 30
"""
...
def parsear_argumentos() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Paso 4: entrenar modelo LSTM.")
    ...
    parser.add_argument("--tasa-aprendizaje", type=float, default=2e-5)
...
def main() -> int:
    ...
    with crear_registrador(cfg.carpeta_resultados, f"4-entrenar_lstm_{cfg.etiqueta_duracion}s_{etiqueta}.txt") as registro:
        titulo(registro, "Paso 4 - Entrenamiento LSTM")
        ...
        try:
            resultado = entrenar_modelo(
                ...
                semilla=cfg.semilla,
                ruta_modelo=cfg.carpeta_resultados / f"modelo_lstm_{cfg.etiqueta_duracion}s_{etiqueta}.pt",
            )
        except ModuleNotFoundError as exc:
            registro.escribir(str(exc))
            registro.escribir("Entrenamiento LSTM omitido hasta instalar dependencias.")
            return 2
        ...
        archivo_metricas = cfg.carpeta_resultados / f"4-metricas_lstm_{cfg.etiqueta_duracion}s_{etiqueta}.txt"
        archivo_metricas.write_text(resumen + "\n", encoding="utf-8")
        registro.escribir(f"Metricas guardadas en: {archivo_metricas}")
        registro.escribir(f"Modelo guardado en: {cfg.carpeta_resultados / f'modelo_lstm_{cfg.etiqueta_duracion}s_{etiqueta}.pt'}")
        registro.escribir("")
        registro.escribir("Paso siguiente: ejecutar 5-evaluar_modelos.py")
    return 0
