...
def parsear_argumentos() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "CSV de rasgos Deep ONMF ajustados con columnas clase, archivo, ruta y F_*. "
            "Si se indica, sustituye a las 7 columnas SBV base en la comparacion."
        ),
    )
...
def seleccionar_rasgos_deep_onmf(
    ruta_csv: Path,
    limite_por_clase: int,
) -> tuple[pd.DataFrame, list[RegistroComparacion], list[str]]:
    ruta = ruta_csv.expanduser().resolve()
    if not ruta.exists():
        raise FileNotFoundError(f"No existe el CSV de rasgos Deep ONMF ajustados: {ruta}")

    df = pd.read_csv(ruta)
    columnas_rasgos = [columna for columna in df.columns if columna.startswith("F_")]
    columnas_necesarias = {"clase", "archivo", "ruta"}
    faltantes = columnas_necesarias.difference(df.columns)
    if faltantes or not columnas_rasgos:
        raise ValueError(
            f"El CSV ajustado debe tener clase, archivo, ruta y columnas F_*. "
            f"Faltantes={sorted(faltantes)}; rasgos={len(columnas_rasgos)}"
        )
    ...
    if not registros:
        raise FileNotFoundError("Las rutas WAV del CSV Deep ONMF ajustado no existen.")
    return df.loc[filas_validas].reset_index(drop=True), registros, columnas_rasgos
...
def analisis_metodo(metricas: pd.DataFrame, metodo: str) -> str:
    ...
    fila_cnn = fila_metricas(metricas, "CNN")
    if (
        metodo == "Deep ONMF"
        and fila["silhouette_tsne"] > fila_cnn["silhouette_tsne"]
        and fila["davies_bouldin_tsne"] < fila_cnn["davies_bouldin_tsne"]
    ):
        observacion_deep = (
            "Deep ONMF usa el perfil ajustado guardado en la carpeta de barrido. En la foto local "
            "las clases forman grupos mas compactos y alejados que en la prueba SBV base; ademas la "
            "tabla coloca a Deep ONMF por delante de CNN en las dos metricas t-SNE y en "
            "Davies-Bouldin de rasgos."
        )
    else:
        observacion_deep = (
...
def guardar_origenes(
    ...
    datos = {
        ...
        "rasgos_deep_onmf_ajustados": str(args.ruta_rasgos_deep_onmf) if args.ruta_rasgos_deep_onmf else None,
...
def main() -> int:
    ...
    if args.ruta_rasgos_deep_onmf is not None:
        ruta_sbv = args.ruta_rasgos_deep_onmf.expanduser().resolve()
        deep_df, registros, columnas_deep = seleccionar_rasgos_deep_onmf(ruta_sbv, args.limite_por_clase)
    else:
        ...
        columnas_deep = [f"SBV_{indice}" for indice in range(1, 8)]
    ...
    matrices["Deep ONMF"] = deep_df[columnas_deep].to_numpy(dtype=np.float64)
